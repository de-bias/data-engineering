---
title: "01_pw_location-device_hour_frequency"
format: html
editor: visual
---
...
if (month == TRUE) {
  optimal_parts <- choose_partitions(file.path(wd, "mapp1/aux/geocoded-hex10-month_aux"), num_cores)
} else {
  optimal_parts <- choose_partitions(file.path(wd, "mapp1/aux/geocoded-hex10_aux"), num_cores)
}
...
Code below identifies unique device-location pairs, and retains night hours.
...
if (month == TRUE) {
  output_path <- file.path(wd, "mapp1", "home" , "device-locations_22t07-month")
  if (dir_exists(output_path)) fs::dir_delete(output_path)
} else {
  output_path <- file.path(wd, "mapp1", "home" , "device-locations_22t07")
  if (dir_exists(output_path)) fs::dir_delete(output_path)
}
