...
sdf_lsoa <- st_read("/Volumes/DEBIAS/data/inputs/geographies/boundaries/LSOA_Dec_2021_EW_BFC_V10.gpkg") %>% 
  st_transform(crs = 4326) %>% 
  select(c("LSOA21CD", "SHAPE")) %>%
...
if (month == TRUE) {
  input_path <- file.path(wd, "mapp1", "home", "location-home_month")
} else {
  input_path <- file.path(wd, "mapp1", "home", "location-home")
}
...
if (month == TRUE) {
  st_write(sdf_hex3_lad, "/Volumes/DEBIAS/data/inputs/mapp1/locations-home/lad/home-location-count_lda-month.gpkg")
} else {
  st_write(sdf_hex3_lad, "/Volumes/DEBIAS/data/inputs/mapp1/locations-home/lad/home-location-count_lda.gpkg")
}
...
if (month == TRUE) {
  st_write(sdf_hex3_msoa, "/Volumes/DEBIAS/data/inputs/mapp1/locations-home/msoa/home-location-count_msoa-month.gpkg")
} else {
  st_write(sdf_hex3_msoa, "/Volumes/DEBIAS/data/inputs/mapp1/locations-home/msoa/home-location-count_msoa.gpkg")
}


# Aggregate into LSOA

sdf_hex3 <- st_join(sdf_lsoa, 
                    sdf_home_location,
                    join = st_contains) 

sdf_hex3_lsoa <- sdf_hex3 %>% 
  group_by(code) %>% 
  summarise( work_location_count = n( ))

if (month == TRUE) {
  st_write(sdf_hex3_lsoa, "/Volumes/DEBIAS/data/inputs/mapp1/locations-home/lsoa/home-location-count_lsoa-month.gpkg")
} else {
  st_write(sdf_hex3_lsoa, "/Volumes/DEBIAS/data/inputs/mapp1/locations-home/lsoa/home-location-count_lsoa.gpkg")
}
```
