...
```{r}
parquet_path <- file.path(wd, "geocoded-hex10-parquet-test")
...
df_csv <- spark_read_csv(
  sc,
  path = file.path(wd, "geocoded-hex10_test", "*", "*-geocoded.csv.gz"),
  memory = TRUE
) %>%
  select(device_aid, code_h3, hour) %>%
  mutate(day = regexp_extract(input_file_name(), ".*/geocoded-hex10_test/([^/]+)/.*", 1))
