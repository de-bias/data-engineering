...
FB gridded movement data for March (excluding weekends).
...
average_gridded_mov_df <- selected_gridded_data  %>%
  ...
  summarise(month_avg_fb_mov = mean(n_crisis, na.rm = TRUE), .groups = "drop")
```
...
average_lad_start_mov <- st_join(lad_sdf,
                    ...
                    join = st_contains) %>%
  select("LAD21CD", "end_coord", "month_avg_fb_mov") %>%
...
mov_lad_tts <- st_join(lad_sdf,
                    average_gridded_mov_sdf,
                    join = st_contains) %>%
  select("LAD21CD", "LAD21CD_start", "month_avg_fb_mov") %>%
  rename("LAD21CD_end" = "LAD21CD") %>%
  st_drop_geometry() %>%
  group_by( LAD21CD_start, LAD21CD_end ) %>%
  summarise(month_avg_fb_mov = sum(month_avg_fb_mov, na.rm = FALSE), .groups = "drop")
...
# aggregate data
 (sum(is.na(mov_lad_tts$month_avg_fb_mov)) / nrow(mov_lad_tts)) * 100
```
...
mov_lad_stt <- aggregate_lad_df %>%
  group_by( LAD21CD_start, LAD21CD_end ) %>%
  summarise(month_avg_fb_mov = mean(n_crisis_LAD, na.rm = TRUE), .groups = "drop")
```
...
# original input data
 (sum(is.na(selected_gridded_data$n_crisis)) / nrow(selected_gridded_data)) * 100
# aggregate data
 (sum(is.na(mov_lad_stt$month_avg_fb_mov)) / nrow(mov_lad_stt)) * 100
```
...
joined_df <- mov_lad_tts %>%
  ...
  rename("month_avg_fb_mov_tts" = "month_avg_fb_mov.x",
         "month_avg_fb_mov_stt" = "month_avg_fb_mov.y") 

plot(joined_df$month_avg_fb_mov_tts, joined_df$month_avg_fb_mov_stt)
```
