...
input_path_home <- file.path(wd, "home", "location-home")
...
input_path_work <- file.path(wd, "work", "location-work")
...
coordinates <- h3_to_geo(df_hw$location_work) %>% as_data_frame()
sdf_work_location <- cbind(df_hw, coordinates) %>% 
  st_as_sf( coords = c("lng", "lat") , crs = 4326)
...
write.csv(df_flows, "/Volumes/DEBIAS/data/outputs/mapp1/flows/lad/flow-home-work_lad_htw_pw.csv")
```
