...
# filter night-time population: 4 PM pacific time is 0 AM UK time
selected_gridded_data <- subset(selected_gridded_data, format(selected_gridded_data$timestamp, "%H:%M") == "16:00")
...
average_gridded_pop_df <- selected_gridded_data %>%
  ...
  summarise(month_avg_fb_pop = mean(n_crisis, na.rm = TRUE))
...
# aggregate data
 (sum(is.na(average_gridded_pop_df$month_avg_fb_pop)) / nrow(average_gridded_pop_df)) * 100
```
...
pop_map <- ggplot() +
  geom_sf(data = lda_shp, fill = "grey50", color = "black") +
  geom_sf(data = average_gridded_pop_sf, aes(colour = month_avg_fb_pop) ) +
...
lad_fb_pop_sf <- fb_pop_sf %>%
  group_by( LAD21NM ) %>%
  summarise(pop_dfd = round(sum(month_avg_fb_pop, na.rm = TRUE)) ) %>% 
  st_drop_geometry()
```
...
lad_fb_pop_sf2 <- lad_fb_pop_sfb %>%
  mutate(pop_dfdb = ifelse(pop_dfdb == 0, NA, pop_dfdb)) %>%
  group_by( LAD21NM ) %>%
