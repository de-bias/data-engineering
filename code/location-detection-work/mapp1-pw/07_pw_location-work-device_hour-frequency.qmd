...
# parquet_path <- file.path(wd, "geocoded-hex10")
...
#   sc,
#   path = file.path(wd, "/aux/geocoded-hex10_aux", "*", "*-geocoded.csv.gz"),
#   memory = FALSE
#   ) %>%
...
```{r}
parquet_path <- file.path(wd, "geocoded-hex10-test")
...
num_cores <- parallel::detectCores()
optimal_parts <- choose_partitions(file.path(wd, "geocoded-hex10-test"), num_cores)
optimal_parts <- as.integer(optimal_parts)   # <<--- Safety, force integer again
...
df_weekdays <- df_all %>%
  mutate(
    ...
    day_of_week = date_format(day_date, "u")  
  ) %>%
...
output_path <- file.path(wd, "work" , "device-locations_09t16")
