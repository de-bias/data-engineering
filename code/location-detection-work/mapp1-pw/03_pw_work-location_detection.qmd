---
title: "Identifying work location"
format: html
editor: visual
---
...
if (month == TRUE) {
  input_path <- file.path(wd, "mapp1", "work", "device-locations-work_time-aggregation_month")
} else {
  input_path <- file.path(wd, "mapp1", "work", "device-locations-work_time-aggregation")
}
...
# Filter for rows where 'is_work' is TRUE and repartition the DataFrame
rdd_2 <- rdd_1 %>% 
  filter(is_work == TRUE) %>% 
  sdf_repartition(partitions = 1)

if (month == TRUE) {
  output_path <- file.path(wd, "mapp1", "work", "location-work_month")
  if (dir_exists(output_path)) dir_delete(output_path) 
} else {
  output_path <- file.path(wd, "mapp1", "work", "location-work")
  if (dir_exists(output_path)) dir_delete(output_path) 
}
