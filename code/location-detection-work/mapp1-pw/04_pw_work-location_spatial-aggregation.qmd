...
sdf_lsoa <- st_read("/Volumes/DEBIAS/data/inputs/geographies/boundaries/Lower_layer_Super_Output_Areas_(December_2021)_Boundaries_EW_BFC_(V10).geojson")  %>% 
  st_transform(crs = 4326) %>% 
...
```{r}
coordinates <- h3_to_geo(df_work_location$location) %>% as_tibble()
...
if (month == TRUE) {
  st_write(sdf_hex3_lsoa, "/Volumes/DEBIAS/data/inputs/mapp1/locations-work/lsoa/work-location-count_lsoa-month.gpkg")
} else {
  st_write(sdf_hex3_lsoa, "/Volumes/DEBIAS/data/inputs/mapp1/locations-work/lsoa/work-location-count_lsoa.gpkg")
}
