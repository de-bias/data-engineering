...
base_folder <- paste0(wd, "/data/inputs/pickwell/uk/converted_parquet_2")  

# Get a list of all subfolders (representing days)
date_folders <- list.dirs(base_folder, recursive = FALSE)


csv_ds <- open_dataset("/Volumes/rdm04/DEBIAS/data/inputs/pickwell/uk/01042021", format = "csv", delimiter = ",")
...
for (folder in c(date_folders[1])){
  ...
  for (file in files_date){
    data <- open_dataset(paste0(folder, "/", file), format="parquet")    
  }
}
...
```{r}
data_parquet <- read_parquet("/Volumes/rdm04/DEBIAS/data/inputs/pickwell/uk/converted_parquet/day=1042021/part-0.parquet", as_data_frame=FALSE)
```
